...
public class GseaPreranked_Rerank extends AbstractGseaTool {
    ...
    public static void main(String[] args) {
        ...
        // -gmx Data/Test/Input/test.gmt -norm meandiv -nperm 1000 -rnk Data/Test/Input/mcf7_test/rnk_files/GSE4773_DEG_Expt1_Control_vs_Group1_gene.rnk -scoring_scheme weighted -make_sets true -plot_top_x 0 -rnd_seed timestamp -set_max 500 -set_min 15 -zip_report false -out Data/Test/Output/ -gui false -anno Data/Test/Input/mcf7_test/annot_file_v2_mcf7.csv -filt Data/Test/Input/mcf7_test/filter_MCF7_estradiol.csv
        ...
        if (annotationFilename != null && filterFilename != null) {
            ...
            HashMap<String, Integer[]> dist = get_dist(annotationFilename, filterFilename);
    ...
    private static HashMap<String, Integer[]> get_dist(String annotationFilename, String filterFilename) {
        HashMap<String, Integer[]> dist = new HashMap<>();

        Iterable<CSVRecord> annotations = null;
        int num_rnk_files_used = 0;
        try {
            for(CSVRecord record: CSVFormat.EXCEL.withHeader().parse(new FileReader(annotationFilename))){
                if (check_filter(filterFilename, record)) {
                    try {
                        Stream<String> lines = Files.lines(Paths.get(record.get("File Name")), Charset.defaultCharset());
                        num_rnk_files_used++;
                        int rank = 1;
        ...
        log.warn(String.format("Used %d rnk files", num_rnk_files_used));
        return dist;
    }
    ...
    private static Boolean check_filter(String filterFilename, CSVRecord record){
        boolean get_file = true;
        try {

            for (CSVRecord var : CSVFormat.EXCEL.withHeader().parse(new FileReader(filterFilename))) {
                String var_name = var.get("Var_Name");
                String var_value = var.get("Var_Value");

                if (record.get(var_name) != null) {
                    String[] record_value = record.get(var_name).split(";");
                    if (!Arrays.asList(record_value).contains(var_value)) {
                        get_file = false;
                    }
                }
            }
